...
| 3 | 2026-09-17 20:2x → | **the run of record** — relaunched fully detached (`setsid`, unbuffered stdout), same tagged protocol, same fetched bytes (fetch NOT repeated; attempts 1–2 never touched the manifest or CSVs), zero configuration changes. |
...
*(appended when the run of record completes — published unchanged, whatever
it says)*
